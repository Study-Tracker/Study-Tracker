...
public class StudyLinkControllerTests {
  ...
  public void fetchStudyLinksTest() throws Exception {
    mockMvc
        .perform(get("/api/internal/study/CPA-10001/links").with(user(username)).with(csrf()))
    ...
    mockMvc
        .perform(get("/api/internal/study/CPA-XXXX/links").with(user(username)).with(csrf()))
        .andExpect(status().isNotFound());

    mockMvc
        .perform(get("/api/internal/study/PPB-10001/links").with(user(username)).with(csrf()))
        .andExpect(status().isOk())
  ...
  public void createStudyLinkTest() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/internal/study/CPA-10001/links")
  ...
  public void deleteStudyLinkTest() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/internal/study/CPA-10001/links/" + link.getId())
