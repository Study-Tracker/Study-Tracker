...
public class ActivityControllerTests {
  ...
  public void getAllActivityTest() throws Exception {
    ...
    mockMvc
        .perform(get("/api/internal/activity").with(user(username)).with(csrf()))
  ...
  public void getSortedActivityTest() throws Exception {
    ...
    mockMvc
        .perform(get("/api/internal/activity?sort=date,desc").with(user(username)).with(csrf()))
        .andDo(MockMvcResultHandlers.print())
        .andExpect(status().isOk())
  ...
  public void getPagedActivityTest() throws Exception {
    ...
    mockMvc
        .perform(get("/api/internal/activity?size=2&page=0&sort=date,desc").with(user(username)).with(csrf()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasKey("content")))
