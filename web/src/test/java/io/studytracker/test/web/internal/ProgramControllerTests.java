...
public class ProgramControllerTests {
  ...
  public void allProgramsTest() throws Exception {
    mockMvc
        .perform(get("/api/internal/program").with(user(username)).with(csrf()))
  ...
  public void findProgramById() throws Exception {
    ...
    mockMvc
        .perform(get("/api/internal/program/" + program.getId()).with(user(username)).with(csrf()))
        .andDo(MockMvcResultHandlers.print())
        .andExpect(status().isOk())
  ...
  public void findNonExistantProgramTest() throws Exception {
    mockMvc
        .perform(get("/api/internal/program/999999").with(user(username)).with(csrf()))
  ...
  public void updateProgramTest() throws Exception {
    ...
    mockMvc
        .perform(get("/api/internal/program/" + program.getId()).with(user(user.getEmail())).with(csrf()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasKey("active")))
        .andExpect(jsonPath("$.active", is(true)));
  ...
  public void deleteProgramTest() throws Exception {
    ...
    User user = userRepository.findByEmail("rblack@email.com").orElseThrow(RecordNotFoundException::new);
    mockMvc
        .perform(delete("/api/internal/program/" + program.getId()).with(user(user.getEmail())).with(csrf()))
        .andExpect(status().isOk());
    program =
        programRepository
            .findByName("Clinical Program A")
  ...
  public void deleteWithoutAuthorizationTest() throws Exception {
    Program program =
        programRepository
            .findByName("Clinical Program A")
            .orElseThrow(RecordNotFoundException::new);
    mockMvc.perform(delete("/api/internal/program/" + program.getId())
        .with(csrf()))
        .andExpect(status().is3xxRedirection());
  }
}
