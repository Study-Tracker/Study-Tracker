...
INSERT INTO file_storage_locations (id, integration_instance_id, type, display_name, name, path, permissions, default_data_location, default_study_location)
VALUES (nextval('hibernate_sequence'), (select max(id) from integration_instances), 'LOCAL_FILE_SYSTEM', 'PLACEHOLDER_FILE_STORE', 'PLACEHOLDER_FILE_STORE', '', 'READ_ONLY', false, false);
...
ALTER TABLE integration_configuration_schema_fields
    ADD CONSTRAINT FK_INTEGRATIONCONFIGURATIONSCHEMAFIELDS_ON_INTEGRATIONDEFINITIO FOREIGN KEY (integration_definition_id) REFERENCES integration_definitions (id);
...
-- Egnyte
-- INSERT INTO integration_definitions (id, name, active, version)
...
-- VALUES (nextval('hibernate_sequence'), 'Tenant Name', 'tenant-name', 'STRING', true, 'Tenant name, as it appears in your tenant URL. For example, if you access Egnyte at https://myorg.egnyte.com, then your tenant name is: "myorg"', true, 1, (select max(id) from integration_definitions));
-- INSERT INTO integration_configuration_schema_fields (id, display_name, field_name, type, required, description, active, "order", supported_integration_id)
-- VALUES (nextval('hibernate_sequence'), 'API Token', 'api-token', 'STRING', true, 'API token for making requests to the Egnyte API. This can be generated in the Developer portal.', true, 2, (select max(id) from integration_definitions));
-- INSERT INTO integration_configuration_schema_fields (id, display_name, field_name, type, required, description, active, "order", supported_integration_id)
-- VALUES (nextval('hibernate_sequence'), 'Root Path', 'root-path', 'STRING', true, 'Root folder to use for storing files. This folder must exist in Egnyte.', true, 3, (select max(id) from integration_definitions));